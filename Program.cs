        ...
        */
        ...
        public static int SearchInsert(int[] nums, int target)
        {
            try
            {
                ...
                int j = nums.Length - 1;
                int ans = -1;
                //Looping for binary search
                while (i <= j)
                {
                    //Calculating the middle element                         
                    int mid = (i + j) / 2;
                    //Checking if mid is equal to targer            
        ...
        */
        ...
        public static string MostCommonWord(string paragraph, string[] banned)
        {
            try
            {
                ...
                // Mapping - Initialized a dictionary
                Dictionary<string, int> map = new Dictionary<string, int>();
                string temp = "";
                //Loop over sentence to traverse character by character
                for (int i = 0; i < paragraph.Length; i++)
                {
                    // Punctuation check - Ignores characters if the condition is satisfied                   
                    if (paragraph[i] == ',') continue;
                    //Condition to check and store words in a temp variable                    
                    if (paragraph[i] == ' ' || paragraph[i] == '.')
                    {
                        //Utilizing the user defined "convertLower" function to convert words into lowercase         
                        string new_string = convertLower(temp);
                        //Increment the word count if it's already present in the map        
                        ...
                        {
                            //Else, enter a new value in to the map
                            map[new_string] = 1;
                        }
                        temp = "";
                    }
                ...
                //Loop to traverse for banned words
                for (int i = 0; i < banned.Length; i++)
                {
                    if (map.ContainsKey(convertLower(banned[i])))
                    {
                        map[convertLower(banned[i])] = 0;
                    }
                }
                var val = map.Keys.ToList();
                int max_fre = 0;
                string ans = "";
                //Traversing to find the most repeated character
        ...
        public static int FindLucky(int[] arr)
        {
            try
            {
                //Initializing a dictionary to create mapping
                Dictionary<int, int> lucky_number = new Dictionary<int, int>();
                //Traversing over arr for mapping the value with its frequency
                ...
                int ans = -1;
                //Condition check for lucky numbers
                foreach (var key in val)
                {
        ...
        */
        ...
        public static string GetHint(string secret, string guess)
        {
            try
            {
                ...
                //Traversing over secret string
                for (int i = 0; i < secret.Length; i++)
                {
                    // increment A, if value matches                
                    if (secret[i] == guess[i])
                    {
                        x++;
                        same_value[i] = true;
                    ...
                    {
                        //Assigning the value to "false" to revisit in next loop
                        same_value[i] = false;
                        //Creation of map                          
                        ...
                        {
                            //Creating new one, if not found in map
                            map[secret[i]] = 1;
                        }
                    }
                }
                ...
                // }
                //Traversing over guess
                for (int i = 0; i < guess.Length; i++)
                {

                    if (same_value[i]) continue;
                    //If the value is set to true in previous step, skip this step                    
                ...
                //Concatenating string to match with the answer
                string ans = x + "A" + y + "B";
                return ans;
            }
            catch (Exception)
        ...
        */
        ...
        public static List<int> NumberOfLines(int[] widths, string s)
        {
            try
            {
                ...
                //Trsversing over each character in array
                foreach (Char _c in c)
                {
                    //Getting the ASCII Value
                    temp = widths[_c - 97];
                    //Condition check if the sum exceeds 100
                    if (sum + temp > 100)
                    {
                        row++;
                        sum = temp;
        ...
        */
        ...
        public static bool IsValid(string bulls_string10)
        {
            ...
            try
            {
                ...
                //Traversing over all characters
                for (int i = 0; i < bulls_string10.Length; i++)
                {
                    //If the symbol is open, push into stack                                 
                    if (bulls_string10[i] == '(' || bulls_string10[i] == '{' || bulls_string10[i] == '[')
                    {
                        st.Push(bulls_string10[i]);
                    }
                    //Pull from the stack, if the encountered symbol is closed
                    if (bulls_string10[i] == ')' || bulls_string10[i] == '}' || bulls_string10[i] == ']')
                    {
                        if (st.Count <= 0)
                        {
